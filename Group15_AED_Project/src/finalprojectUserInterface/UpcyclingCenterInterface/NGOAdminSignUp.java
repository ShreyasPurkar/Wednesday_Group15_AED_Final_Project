...
public class NGOAdminSignUp extends javax.swing.JPanel {
        ...
        try {
            if (EmpytyFieldValidation()) {
                if (RegexValidation()) {
                    ...
                    UpcyclingCenterClass hosp = new UpcyclingCenterClass(address, uname_txt.getText(), pswd_txt.getText(), name_txt.getText(), reg_txt.getText());
                    System.out.println("NGO" + hosp.getpName());
                    if (operatingSystem.isUpcyclingAdminPresent(uname_txt.getText())) {
                        JOptionPane.showMessageDialog(this, "Upcycling center admin with given username already exists.");
                        return;
                    }
                    operatingSystem.addUpcyclingCenter(hosp);
    ...
    private boolean RegexValidation() {
        ...
        if (!addr_txt.getText().matches("^[a-zA-Z 0-9\\-]+$")) {
            addr_txt.setBorder(BorderFactory.createLineBorder(Color.RED, 1));
            addr_txt.setToolTipText("Please enter only characters, numbers, space and symbol like -.");
            validationCheck = false;
        }

        if (addr_txt.getText().matches("^[a-zA-Z 0-9\\-]+$")) {
            addr_txt.setBorder(BorderFactory.createLineBorder(Color.BLUE, 1));
            addr_txt.setToolTipText(null);
        }

        if (!cityname_txt.getText().matches("^[a-zA-Z ]+$")) {
            cityname_txt.setBorder(BorderFactory.createLineBorder(Color.RED, 1));
            cityname_txt.setToolTipText("Please enter only characters.");
            validationCheck = false;
        }

        if (cityname_txt.getText().matches("^[a-zA-Z ]+$")) {
            cityname_txt.setBorder(BorderFactory.createLineBorder(Color.BLUE, 1));
            cityname_txt.setToolTipText(null);
        }
