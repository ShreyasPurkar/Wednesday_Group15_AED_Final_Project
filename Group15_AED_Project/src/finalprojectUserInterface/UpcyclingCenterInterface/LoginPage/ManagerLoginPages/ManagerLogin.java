 ...
 */
package finalprojectUserInterface.UpcyclingCenterInterface.LoginPage.ManagerLoginPages;
...
public class ManagerLogin extends javax.swing.JPanel {
    ...
    UpcyclingCenterManager manager;
    String org;

    public ManagerLogin(MainJFrameForm MainLPage, DB4OUtility dB4OUtility, OperatingSystem operatingSystem, UpcyclingCenterManager d) {
        initComponents();
        this.MainLoginPage = MainLPage;
        this.dB4OUtility = dB4OUtility;
        this.operatingSystem = operatingSystem;
        this.manager = d;
        for (UpcyclingCenterClass n : operatingSystem.getUpcyclingDirectory().getListOfNGO()) {
...
//        populateRecieverTable();
    ...
    private void initComponents() {
        ...
        statustxt.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Upcycled", "Rejected" }));
        jPanel3.add(statustxt, new org.netbeans.lib.awtextra.AbsoluteConstraints(90, 670, 230, -1));
...
//    }
    private void populateTable(String name) {
        ...
        for (AssignmentDonation d : operatingSystem.getAssignmentDonationList()) {
            if (d.getStatusDonation().contentEquals("Delivered for Upcycling") && d.getUpManager().getNGOName().equals(org)
                    && d.getUpManager().getpName().equals(manager.getpName())) {

                try {
                    if (d.getUpManager().getpName().matches(name)) {
    ...
    private void populateTableName(String name, String item) {
        ...
        for (AssignmentDonation d : operatingSystem.getAssignmentDonationList()) {
            if (d.getStatusDonation().contentEquals("Delivered for Upcycling") && d.getUpManager().getNGOName().equals(org)
                ...
                try {
                    if (d.getUpManager().getpName().matches(name) && d.getEntityName().contains(item)) {
                        Object[] row = new Object[11];
                        row[0] = d.getEntityName();
                        row[1] = d.getCategory();
                        row[2] = d.getUnits();
