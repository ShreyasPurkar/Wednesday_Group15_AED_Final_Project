...
public class Visualization extends javax.swing.JPanel {
    ...
    private ChartPanel chartPanelCategoryEntity() {
        ...
        Map<String, Integer> hashMap = new HashMap();
        for(AssignmentDonation dasn: operatingSystem.getAssignmentDonationList()){
            if (dasn.getStatusDonation().contentEquals("Recycled") || dasn.getStatusDonation().contentEquals("Upcycled") || dasn.getStatusDonation().contentEquals("Requested for Upcycling") || dasn.getStatusDonation().contentEquals("Delivered for Upcycling")){
                String key = dasn.getCategory().toLowerCase();
                Integer count = (int)dasn.getUnits();

                hashMap.put(key,  hashMap.containsKey(key)? count +hashMap.get(key):count);
        ...
        JFreeChart chart = ChartFactory.createBarChart("", "CATEGORY",
            "Weight in Lbs", defaultCatDataset, PlotOrientation.VERTICAL, false, false, false);
        return new ChartPanel(chart) {
                @Override
                public Dimension getPreferredSize() {
                    return new Dimension(N * 2, N * 2);
     ...
     private ChartPanel chartPanelEmissionEntity() {
        final XYSeries series = new XYSeries("Data");
        for (int i = 0; i < random.nextInt(N) + N / 2; i++) {
                series.add(i, random.nextGaussian());
            }
        ...
        for(AssignmentDonation dasn: operatingSystem.getAssignmentDonationList()){
            if (dasn.getStatusDonation().contentEquals("Recycled") || dasn.getStatusDonation().contentEquals("Upcycled") || dasn.getStatusDonation().contentEquals("Requested for Upcycling") || dasn.getStatusDonation().contentEquals("Delivered for Upcycling")){
                String key = dasn.getCategory().toLowerCase();
                Integer count = (int)dasn.getUnits();
                if (key.contentEquals("plastic")) count = (int)dasn.getUnits()*3;
                if (key.contentEquals("paper")) count = (int)dasn.getUnits()*2;
                if (key.contentEquals("electronics")) count = (int)dasn.getUnits()*4;
                if (key.contentEquals("metal")) count = (int)dasn.getUnits()*5;
        ...
        JFreeChart chart = ChartFactory.createBarChart("", "CATEGORY",
            "Carbon Reduction in Lbs", defaultCatDataset, PlotOrientation.VERTICAL, false, false, false);
        return new ChartPanel(chart) {
                @Override
                public Dimension getPreferredSize() {
                    return new Dimension(N * 2, N * 2);
     ...
     private ChartPanel chartPanelLandEntity() {
        final XYSeries series = new XYSeries("Data");
        for (int i = 0; i < random.nextInt(N) + N / 2; i++) {
                series.add(i, random.nextGaussian());
            }
        ...
        for(AssignmentDonation dasn: operatingSystem.getAssignmentDonationList()){
            if (dasn.getStatusDonation().contentEquals("Recycled") || dasn.getStatusDonation().contentEquals("Upcycled") || dasn.getStatusDonation().contentEquals("Requested for Upcycling") || dasn.getStatusDonation().contentEquals("Delivered for Upcycling")){
                String key = dasn.getCategory().toLowerCase();
                Integer count = (int)dasn.getUnits()*1000;
                if (key.contentEquals("plastic")) count = (int)dasn.getUnits()*2000;
                if (key.contentEquals("paper")) count = (int)dasn.getUnits()*1500;
                if (key.contentEquals("electronics")) count = (int)dasn.getUnits()*3000;
                if (key.contentEquals("metal")) count = (int)dasn.getUnits()*4000;
        ...
        JFreeChart chart = ChartFactory.createBarChart("", "CATEGORY",
            "Land Filled Saved in cm^3", defaultCatDataset, PlotOrientation.VERTICAL, false, false, false);
        return new ChartPanel(chart) {
                @Override
                public Dimension getPreferredSize() {
                    return new Dimension(N * 2, N * 2);
    ...
    private ChartPanel chartPanelStatusPanel() {
        ...
        for(AssignmentDonation dasn: operatingSystem.getAssignmentDonationList()){
            if (dasn.getStatusDonation().contentEquals("Available") || dasn.getStatusDonation().contentEquals("Recycled") || dasn.getStatusDonation().contentEquals("Requested for Recycling") || dasn.getStatusDonation().contentEquals("Delivered for Recycling")  || dasn.getStatusDonation().contentEquals("Upcycled") || dasn.getStatusDonation().contentEquals("Requested for Upcycling") || dasn.getStatusDonation().contentEquals("Delivered for Upcycling") || dasn.getStatusDonation().contentEquals("Rejected") || dasn.getStatusDonation().contentEquals("Not Deliverable") ) {
            try{
...
//    }
    ...
    chartPanelContainer.add(chartPanelEmissionEntity());
    chartPanelContainer.add(chartPanelLandEntity());
